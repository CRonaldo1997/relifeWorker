...
# ── Profile avatar (#数字员工头像) ─────────────────────────────────────────
# Per-profile avatar images.  Stored at <home>/avatar.<ext> where <home> is
# either the root home (default profile) or ~/.hermes/profiles/<name>.
# Frontend retrieves the image via GET /api/profile/avatar?name=<n>; the URL
# carries an &v=<mtime> cache-buster so freshly uploaded images replace stale
# cached copies in the browser.
_AVATAR_EXTS = ('png', 'jpg', 'jpeg', 'webp', 'gif')
_AVATAR_MAX_BYTES = 5 * 1024 * 1024  # 5 MB hard cap, independent of MAX_UPLOAD_BYTES
_AVATAR_MIME_BY_EXT = {
    'png': 'image/png',
    'jpg': 'image/jpeg',
    'jpeg': 'image/jpeg',
...
def _profile_home_for_avatar(name: str) -> Path:
    """Resolve the home directory under which the profile avatar lives."""
    if not name or _is_root_profile(name):
        return _DEFAULT_HERMES_HOME
    return _resolve_named_profile_home(name)


def get_profile_avatar_path(name: str) -> Optional[Path]:
    """Return the existing avatar Path for *name* or None if not configured."""
    try:
        home = _profile_home_for_avatar(name)
...
def profile_avatar_mime(path: Path) -> str:
    """Return the MIME type for an avatar Path based on its extension."""
    ext = path.suffix.lower().lstrip('.')
    return _AVATAR_MIME_BY_EXT.get(ext, 'application/octet-stream')


def _profile_avatar_url(name: str) -> Optional[str]:
    """Return a cache-busted GET URL for the avatar, or None."""
    p = get_profile_avatar_path(name)
    if not p:
        return None
    try:
    ...
    return f"/api/profile/avatar?name={quote(name or 'default', safe='')}&v={v}"


def set_profile_avatar(name: str, file_bytes: bytes, ext: str) -> Path:
    """Save *file_bytes* as the avatar for profile *name*.

    Removes any prior avatar with a different extension so only one canonical
    file exists per profile.  Returns the destination Path.
    """
    if not file_bytes:
        raise ValueError('Empty avatar payload')
    if len(file_bytes) > _AVATAR_MAX_BYTES:
        raise ValueError(f'Avatar too large (max {_AVATAR_MAX_BYTES // 1024 // 1024} MB)')
    ext = (ext or '').lower().lstrip('.')
    if ext == 'jpg':
        ext = 'jpeg'
    if ext not in _AVATAR_MIME_BY_EXT:
        raise ValueError('Unsupported avatar format; use png, jpg, jpeg, webp, or gif')
    home = _profile_home_for_avatar(name)
    home.mkdir(parents=True, exist_ok=True)
    # Wipe any existing avatar so we never leave stale copies behind.
    for old_ext in _AVATAR_EXTS:
        prior = home / f'avatar.{old_ext}'
        if prior.exists():
            try:
                prior.unlink()
            except OSError:
                logger.debug('Failed to remove prior avatar %s', prior)
    dest = home / f'avatar.{ext}'
    dest.write_bytes(file_bytes)
    return dest


def delete_profile_avatar(name: str) -> bool:
    """Remove the avatar for profile *name*.  Returns True if a file was deleted."""
    try:
        home = _profile_home_for_avatar(name)
    except (ValueError, OSError):
        return False
    removed = False
    for ext in _AVATAR_EXTS:
        prior = home / f'avatar.{ext}'
        if prior.exists():
            try:
                prior.unlink()
                removed = True
            except OSError:
                logger.debug('Failed to remove avatar %s', prior)
    return removed
...
def list_profiles_api() -> list:
    ...
    for p in infos:
        result.append({
            ...
            'avatar_url': _profile_avatar_url(p.name),
        })
    return result
...
def _default_profile_dict() -> dict:
    ...
    return {
        ...
        'skill_count': 0,
        'avatar_url': _profile_avatar_url('default'),
    }
