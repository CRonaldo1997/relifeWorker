...
def handle_profile_avatar_upload(handler):
    """Receive a profile avatar image and persist it under the profile home.

    Multipart fields:
      - name (required) : profile name (or 'default')
      - file (required) : image bytes (png/jpg/jpeg/webp/gif)
    Returns JSON with the cache-busted avatar_url.
    """
    import traceback as _tb
    from api.profiles import set_profile_avatar, _profile_avatar_url, _PROFILE_ID_RE
    try:
        content_type = handler.headers.get('Content-Type', '')
        content_length = int(handler.headers.get('Content-Length', 0) or 0)
        if content_length <= 0:
            return j(handler, {'error': 'Empty upload'}, status=400)
        # 5 MB matches the per-profile cap inside set_profile_avatar; reject
        # oversized streams up front so we don't buffer them in memory.
        avatar_cap = 5 * 1024 * 1024
        if content_length > avatar_cap:
            return j(handler, {'error': f'Avatar too large (max {avatar_cap//1024//1024}MB)'}, status=413)
        fields, files = parse_multipart(handler.rfile, content_type, content_length)
        name = (fields.get('name') or '').strip() or 'default'
        if name != 'default' and not _PROFILE_ID_RE.fullmatch(name):
            return j(handler, {'error': 'Invalid profile name'}, status=400)
        if 'file' not in files:
            return j(handler, {'error': 'No file field in request'}, status=400)
        filename, file_bytes = files['file']
        if not filename or not file_bytes:
            return j(handler, {'error': 'Empty avatar payload'}, status=400)
        ext = (Path(filename).suffix or '').lower().lstrip('.')
        if not ext:
            # fall back to MIME-derived extension when filename had none
            guessed = mimetypes.guess_extension(
                mimetypes.guess_type(filename)[0] or 'application/octet-stream'
            ) or ''
            ext = guessed.lstrip('.').lower()
        dest = set_profile_avatar(name, file_bytes, ext)
        return j(handler, {
            'ok': True,
            'name': name,
            'avatar_url': _profile_avatar_url(name),
            'size': dest.stat().st_size,
        })
    except ValueError as e:
        return j(handler, {'error': str(e)}, status=400)
    except Exception:
        print('[webui] avatar upload error: ' + _tb.format_exc(), flush=True)
        return j(handler, {'error': 'Avatar upload failed'}, status=500)
