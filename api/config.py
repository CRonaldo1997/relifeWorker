...
_SETTINGS_DEFAULTS = {
    ...
    "welcome_prompts": [
        ...
        "今天的日程安排",
        "帮我规划一个小项目",
    ],  # quick-click prompts shown on the empty new-chat screen; editable in Settings → Conversation
...
def save_settings(settings: dict) -> dict:
    ...
    for k, v in settings.items():
        if k in _SETTINGS_ALLOWED_KEYS:
            ...
            # Welcome page quick-prompt suggestions (editable list of short strings)
            if k == "welcome_prompts":
                if not isinstance(v, list):
                    continue
                cleaned = []
                for s in v:
                    ...
                    if not txt:
                        continue
                    # Cap individual prompt length to keep the UI tidy
                    cleaned.append(txt[:200])
                # Cap total count to avoid an unbounded welcome screen
                v = cleaned[:20]
            # Coerce bool keys
