...
def _csrf_exempt_path(path: str) -> bool:
    ...
    return path in {"/api/auth/login", "/api/auth/register", "/api/csp-report"}
...
def _check_csrf(handler) -> bool:
    ...
    from api.user_auth import CSRF_HEADER_NAME, parse_cookie, verify_csrf_token
...
# Light blue/white split layout: left panel with brand text + auth card,
# right side bleeds into a building photo background (`/static/bg.png`).
_AUTH_CSS = """
...
/* semi-transparent white overlay on the left side keeps the card readable */
body::before{
...
.card{
  ...
  border:1px solid rgba(0,87,163,.14);border-radius:36px;padding:50px 60px 40px;
  box-shadow:0 20px 50px -12px rgba(0,87,163,.18),0 40px 90px -25px rgba(0,87,163,.28),
    0 0 0 1px rgba(255,255,255,1) inset;
  backdrop-filter:blur(35px);-webkit-backdrop-filter:blur(35px);
}
.logo-row{width:100%;display:flex;align-items:center;justify-content:center;gap:20px;margin-bottom:28px;}
.logo-img{height:72px;width:auto;object-fit:contain;flex-shrink:0;}
h2{width:100%;font-size:28px;font-weight:800;color:#0D1B2E;letter-spacing:-.02em;margin-bottom:10px;text-align:center;}
.sub{width:100%;font-size:15px;color:#7A8DA0;margin-bottom:32px;line-height:1.5;text-align:center;}
...
input[type=text],input[type=password]{
  ...
  color:#0D1B2E;font-size:14px;font-family:inherit;outline:none;
...
.submit-btn{
  ...
  transition:all .2s cubic-bezier(.16,1,.3,1);letter-spacing:.01em;margin-top:8px;
...
.err{color:#C0392B;font-size:12px;margin-top:12px;display:none;
  background:rgba(192,57,43,.06);border:1px solid rgba(192,57,43,.15);
  border-radius:8px;padding:8px 12px;line-height:1.45;}
.switch-link{text-align:center;margin-top:18px;font-size:13px;color:#7A8DA0;}
...
@media(max-width:640px){
  ...
  .card{margin-top:20px;width:100%;padding:36px 28px;}
}
"""
...
<link rel="icon" type="image/png" href="/static/ai.png">
...
<div class="left-panel">
  ...
  <div class="brand-title">中再寿险数字员工系统</div>
  <div class="card">
    <div class="logo-row">
    ...
    <div class="switch-link">
      没有账号？<a href="/register">创建您的数字员工通行证</a>
    </div>
  </div>
</div>
...
<script src="static/register.js?v={{WEBUI_VERSION}}"></script>
</body></html>"""
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path in ("/", "/index.html") or parsed.path.startswith("/session/"):
        try:
            ...
            csrf_token = ""
            try:
                from api.user_auth import csrf_token_for_session, parse_cookie, verify_session
    ...
    if parsed.path == "/login":
        ...
        try:
            from api.user_auth import (
                ...
                verify_session as _ua_verify,
            )
            _cv = _ua_parse_cookie(handler)
            if _cv and _ua_verify(_cv):
    ...
    if parsed.path == "/api/auth/status":
        from api.user_auth import parse_cookie, verify_session

        cv = parse_cookie(handler)
        session_info = verify_session(cv) if cv else None
    ...
    if parsed.path == "/api/profile/active":
        ...
        return j(
            handler,
            {
                ...
                "avatar_url": _profile_avatar_url(_active_name),
            },
        )
    ...
    if parsed.path == "/api/profile/avatar":
        ...
        name = (qs.get("name", [""])[0] or "").strip() or "default"
        if name != "default" and not _PROFILE_ID_RE.fullmatch(name):
            return bad(handler, "Invalid profile name", 400)
        target = get_profile_avatar_path(name)
        if not target:
            return bad(handler, "Avatar not found", 404)
        return _serve_file_bytes(
            handler,
            target,
            profile_avatar_mime(target),
            "inline",
            "private, max-age=300",
        )
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/profile/avatar":
        from api.upload import handle_profile_avatar_upload
        return handle_profile_avatar_upload(handler)
    ...
    if parsed.path == "/api/profile/avatar/delete":
        name = (body.get("name", "") or "").strip() or "default"
        try:
            from api.profiles import delete_profile_avatar, _PROFILE_ID_RE
            if name != "default" and not _PROFILE_ID_RE.fullmatch(name):
                return bad(handler, "Invalid profile name")
            removed = delete_profile_avatar(name)
    ...
    if parsed.path == "/api/auth/login":
        ...
        cookie_val = create_session(result)  # result = canonical username
        handler.send_response(200)
