...
function _renderProfileDetail(p, activeName){
  ...
  // Avatar (#数字员工头像) — render preview with upload/replace/delete controls.
  const _avatarUrl = p.avatar_url || '';
  const _avatarPreviewHtml = _avatarUrl
    ? `<img src="${esc(_avatarUrl)}" alt="" class="profile-avatar-preview-img">`
    : `<div class="profile-avatar-preview-placeholder">${esc((p.name||'?').charAt(0).toUpperCase())}</div>`;
  const _avatarUploadLabel = _avatarUrl ? t('profile_avatar_change') : t('profile_avatar_upload');
  const _avatarRow = `
    <div class="detail-row">
      <div class="detail-row-label">${esc(t('profile_avatar_label'))}</div>
      <div class="detail-row-value">
        <div class="profile-avatar-editor">
          <div class="profile-avatar-preview">${_avatarPreviewHtml}</div>
          <div class="profile-avatar-actions">
            <button type="button" class="detail-btn" id="btnProfileAvatarUpload">${esc(_avatarUploadLabel)}</button>
            ${_avatarUrl ? `<button type="button" class="detail-btn danger" id="btnProfileAvatarDelete">${esc(t('profile_avatar_delete'))}</button>` : ''}
            <input type="file" id="profileAvatarFileInput" accept="image/png,image/jpeg,image/webp,image/gif" style="display:none">
          </div>
          <div class="detail-form-hint">${esc(t('profile_avatar_hint'))}</div>
        </div>
      </div>
    </div>`;
  const rows = [];
  rows.push(_avatarRow);
  ...
  // Wire avatar editor controls after the body is in the DOM.
  _wireProfileDetailAvatarControls(p);
}

// ── Profile avatar editor (detail view) ─────────────────────────────────
function _wireProfileDetailAvatarControls(p){
  const fileInput = $('profileAvatarFileInput');
  const uploadBtn = $('btnProfileAvatarUpload');
  const deleteBtn = $('btnProfileAvatarDelete');
  if (uploadBtn && fileInput) {
    ...
    fileInput.onchange = async () => {
      ...
      fileInput.value = '';
      if (!f) return;
      await _uploadProfileAvatar(p.name, f, /*isActive=*/ (S.activeProfile === p.name));
    };
  }
  if (deleteBtn) {
    deleteBtn.onclick = async () => {
      const _ok = await showConfirmDialog({
        title: t('profile_avatar_delete_confirm_title'),
        message: t('profile_avatar_delete_confirm_message'),
        confirmLabel: t('delete_title'),
        danger: true,
        focusCancel: true,
      });
      if (!_ok) return;
      await _deleteProfileAvatar(p.name, /*isActive=*/ (S.activeProfile === p.name));
    };
  }
}

function _validateAvatarFile(f){
  if (!f) return t('profile_avatar_invalid_type');
  const allowed = ['image/png','image/jpeg','image/jpg','image/webp','image/gif'];
  const mime = (f.type || '').toLowerCase();
  if (mime && !allowed.includes(mime)) return t('profile_avatar_invalid_type');
  // 5 MB hard cap matches the backend (api/profiles.py _AVATAR_MAX_BYTES).
  if (f.size && f.size > 5 * 1024 * 1024) return t('profile_avatar_too_large');
  return '';
}

async function _uploadProfileAvatar(name, file, isActive){
  const err = _validateAvatarFile(file);
  if (err) { showToast(err); return null; }
  try {
    const fd = new FormData();
    fd.append('name', name || 'default');
    fd.append('file', file, file.name || 'avatar');
    const url = new URL('api/profile/avatar', document.baseURI || location.href).href;
    const res = await fetch(url, { method: 'POST', credentials: 'include', body: fd });
    if (!res.ok) {
      const txt = await res.text().catch(() => '');
      let msg = t('profile_avatar_upload_failed');
      try { const d = JSON.parse(txt || '{}'); if (d && d.error) msg = d.error; } catch(_){}
      throw new Error(msg);
    }
    const data = await res.json();
    if (data && data.error) throw new Error(data.error);
    if (isActive && data && data.avatar_url && typeof setProfileAvatarUrl === 'function') {
      setProfileAvatarUrl(data.avatar_url);
    }
    showToast(t('profile_avatar_updated'));
    // Refresh the profiles list so the detail view reflects the new avatar URL.
    try {
      await loadProfilesPanel();
    } catch (_) {}
    return data;
  } catch (e) {
    showToast((e && e.message) || t('profile_avatar_upload_failed'));
    return null;
  }
}

async function _deleteProfileAvatar(name, isActive){
  try {
    await api('/api/profile/avatar/delete', { method: 'POST', body: JSON.stringify({ name: name || 'default' }) });
    if (isActive && typeof setProfileAvatarUrl === 'function') setProfileAvatarUrl('');
    showToast(t('profile_avatar_deleted'));
    try { await loadProfilesPanel(); } catch (_) {}
    return true;
  } catch (e) {
    showToast((e && e.message) || t('delete_failed'));
    return false;
  }
}
...
async function switchToProfile(name) {
  ...
  try {
    ...
    // Apply the new profile's avatar to chat bubbles immediately so the
    // assistant role icon reflects the freshly switched digital worker.
    try {
      const _switchedProfile = (Array.isArray(data.profiles) ? data.profiles : []).find(pp => pp && pp.name === S.activeProfile);
      const _switchedAvatar = _switchedProfile && _switchedProfile.avatar_url ? _switchedProfile.avatar_url : '';
      if (typeof setProfileAvatarUrl === 'function') setProfileAvatarUrl(_switchedAvatar);
    } catch (_) {}
...
function openProfileCreate(){
  ...
  _profileMode = 'create';
  _pendingProfileFormAvatar = null;
  _renderProfileForm();
}
...
function _renderProfileForm(){
  ...
  body.innerHTML = `
    <div class="main-view-content">
      <form class="detail-form" onsubmit="event.preventDefault(); saveProfileForm();">
        <div class="detail-form-row">
          <label>${esc(t('profile_avatar_label'))}</label>
          <div class="profile-avatar-editor">
            <div class="profile-avatar-preview" id="profileFormAvatarPreview">
              <div class="profile-avatar-preview-placeholder">?</div>
            </div>
            <div class="profile-avatar-actions">
              <button type="button" class="detail-btn" id="btnProfileFormAvatarPick">${esc(t('profile_avatar_upload'))}</button>
              <button type="button" class="detail-btn danger" id="btnProfileFormAvatarClear" style="display:none">${esc(t('profile_avatar_clear'))}</button>
              <input type="file" id="profileFormAvatarFile" accept="image/png,image/jpeg,image/webp,image/gif" style="display:none">
            </div>
            <div class="detail-form-hint">${esc(t('profile_avatar_hint'))}</div>
          </div>
        </div>
  ...
  _wireProfileFormAvatarControls();
  const n = $('profileFormName');
  if (n) n.focus();
  _populateProfileFormModelSelect();
}

// Pending avatar selected during the create-profile form flow. Held in memory
// until /api/profile/create succeeds, then uploaded to /api/profile/avatar.
let _pendingProfileFormAvatar = null;

function _wireProfileFormAvatarControls(){
  const pickBtn = $('btnProfileFormAvatarPick');
  const clearBtn = $('btnProfileFormAvatarClear');
  const fileInput = $('profileFormAvatarFile');
  const preview = $('profileFormAvatarPreview');
  const namePreviewSync = () => {
    if (_pendingProfileFormAvatar) return;
    const nameEl = $('profileFormName');
    const initial = ((nameEl && nameEl.value || '').trim().charAt(0) || '?').toUpperCase();
    if (preview) preview.innerHTML = `<div class="profile-avatar-preview-placeholder">${esc(initial)}</div>`;
  };
  const nameEl = $('profileFormName');
  if (nameEl) nameEl.addEventListener('input', namePreviewSync);
  namePreviewSync();
  if (pickBtn && fileInput) {
    ...
    fileInput.onchange = () => {
      ...
      fileInput.value = '';
      if (!f) return;
      const err = _validateAvatarFile(f);
      if (err) { showToast(err); return; }
      _pendingProfileFormAvatar = f;
      if (pickBtn) pickBtn.textContent = t('profile_avatar_change');
      if (clearBtn) clearBtn.style.display = '';
      if (preview) {
        const objectUrl = URL.createObjectURL(f);
        preview.innerHTML = `<img src="${esc(objectUrl)}" alt="" class="profile-avatar-preview-img">`;
      }
    };
  }
  if (clearBtn) {
    clearBtn.onclick = () => {
      _pendingProfileFormAvatar = null;
      if (pickBtn) pickBtn.textContent = t('profile_avatar_upload');
...
async function saveProfileForm(){
  ...
  try {
    ...
    // Upload the avatar selected in the form (if any) before refreshing the
    // profile list so the freshly rendered detail view shows the new avatar.
    const _pendingAvatarFile = _pendingProfileFormAvatar;
    _pendingProfileFormAvatar = null;
    if (_pendingAvatarFile) {
      try {
        await _uploadProfileAvatar(name, _pendingAvatarFile, /*isActive=*/ false);
      } catch (_) { /* surface error already toasted by helper */ }
    }
    _invalidateKanbanProfileCache();
    _profilePreFormDetail = null;
    await loadProfilesPanel();
...
let _settingsWelcomePromptsAutosaveTimer = null;
let _settingsWelcomePromptsAutosaveRetryPayload = null;
let _welcomePromptsState = [];
...
// ── Welcome page prompts editor (Settings → Conversation) ───────────────────
// Mirrors the preferences autosave pattern: 350ms debounce, POSTs the full
// welcome_prompts array to /api/settings, and re-renders the empty-state
// suggestion buttons on the welcome screen so changes are visible immediately.

function _renderWelcomePromptsEditor(){
  const host=$('welcomePromptsEditor');
  if(!host) return;
  host.innerHTML='';
  if(!_welcomePromptsState.length){
    const empty=document.createElement('div');
    empty.style.cssText='font-size:12px;color:var(--muted);padding:8px 4px;';
    empty.setAttribute('data-i18n','settings_welcome_prompts_empty');
    empty.textContent=(typeof t==='function')?t('settings_welcome_prompts_empty'):'No suggestions yet.';
    host.appendChild(empty);
    return;
  }
  _welcomePromptsState.forEach((text,idx)=>{
    const row=document.createElement('div');
    row.className='welcome-prompt-row';
    ...
    input.maxLength=200;
    input.placeholder=(typeof t==='function')?t('settings_welcome_prompts_placeholder'):'Enter prompt text';
    input.setAttribute('data-i18n-placeholder','settings_welcome_prompts_placeholder');
    input.style.cssText='flex:1;padding:8px;background:var(--code-bg);color:var(--text);border:1px solid var(--border2);border-radius:6px;font-size:13px;';
    input.addEventListener('input',function(){
      const i=parseInt(this.dataset.idx,10);
      if(!isNaN(i)) _welcomePromptsState[i]=this.value;
      _scheduleWelcomePromptsAutosave();
    });
    const removeBtn=document.createElement('button');
    removeBtn.type='button';
    removeBtn.className='settings-action-btn danger';
    removeBtn.title=(typeof t==='function')?t('settings_welcome_prompts_remove'):'Remove';
    removeBtn.setAttribute('data-i18n-title','settings_welcome_prompts_remove');
    ...
    removeBtn.onclick=()=>removeWelcomePrompt(idx);
...
function addWelcomePrompt(){
  if(_welcomePromptsState.length>=20) return;
  _welcomePromptsState.push('');
  _renderWelcomePromptsEditor();
  // Focus the newly added input for immediate typing.
  const host=$('welcomePromptsEditor');
  if(host){
...
function removeWelcomePrompt(idx){
  if(idx<0||idx>=_welcomePromptsState.length) return;
  _welcomePromptsState.splice(idx,1);
  _renderWelcomePromptsEditor();
  _scheduleWelcomePromptsAutosave();
}

function _welcomePromptsPayloadFromUi(){
  // Drop empties on save but keep them visible in the editor while the user is typing.
  const cleaned=_welcomePromptsState
    .map(s=>(typeof s==='string'?s:'').trim())
    .filter(s=>s.length>0)
    .slice(0,20);
  return {welcome_prompts: cleaned};
}

function _setWelcomePromptsAutosaveStatus(state){
  const el=$('settingsWelcomePromptsAutosaveStatus');
  if(!el) return;
  el.className='settings-autosave-status';
  if(!state){el.textContent='';return;}
  el.classList.add('is-'+state);
  if(state==='saving') el.textContent=t('settings_autosave_saving');
  else if(state==='saved') el.textContent=t('settings_autosave_saved');
  else if(state==='failed') el.innerHTML=`<span>${esc(t('settings_autosave_failed'))}</span> <button type="button" onclick="_retryWelcomePromptsAutosave()">${esc(t('settings_autosave_retry'))}</button>`;
}

function _scheduleWelcomePromptsAutosave(){
  const payload=_welcomePromptsPayloadFromUi();
  _settingsWelcomePromptsAutosaveRetryPayload=payload;
  _setWelcomePromptsAutosaveStatus('saving');
  if(_settingsWelcomePromptsAutosaveTimer) clearTimeout(_settingsWelcomePromptsAutosaveTimer);
  _settingsWelcomePromptsAutosaveTimer=setTimeout(()=>_autosaveWelcomePromptsSettings(payload),350);
}

async function _autosaveWelcomePromptsSettings(payload){
  try{
    const saved=await api('/api/settings',{method:'POST',body:JSON.stringify(payload)});
    _settingsWelcomePromptsAutosaveRetryPayload=null;
    const next=Array.isArray(saved&&saved.welcome_prompts)?saved.welcome_prompts:[];
    window._welcomePrompts=next.slice();
    if(typeof renderWelcomePrompts==='function') renderWelcomePrompts(next);
    _setWelcomePromptsAutosaveStatus('saved');
  }catch(e){
    console.warn('[settings] welcome_prompts autosave failed', e);
    _setWelcomePromptsAutosaveStatus('failed');
  }
}

function _retryWelcomePromptsAutosave(){
  const payload=_settingsWelcomePromptsAutosaveRetryPayload||_welcomePromptsPayloadFromUi();
  _setWelcomePromptsAutosaveStatus('saving');
  _autosaveWelcomePromptsSettings(payload);
}
...
async function loadSettingsPanel(){
  try{
    ...
    // Welcome page prompts editor (Settings → Conversation)
    _welcomePromptsState=Array.isArray(settings.welcome_prompts)
      ? settings.welcome_prompts.filter(s=>typeof s==='string')
      : [];
    window._welcomePrompts=_welcomePromptsState.slice();
    _renderWelcomePromptsEditor();
    _setWelcomePromptsAutosaveStatus('');
    const showTpsCb=$('settingsShowTps');
