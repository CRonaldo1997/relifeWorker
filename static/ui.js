...
function assistantAvatarSrc(){
  return (S&&S.profileAvatarUrl) ? S.profileAvatarUrl : 'static/ai.png';
}
function setProfileAvatarUrl(url){
  const next=(typeof url==='string'?url:'')||'';
  if(S.profileAvatarUrl===next) return;
  S.profileAvatarUrl=next;
  const src=assistantAvatarSrc();
  try{
    document.querySelectorAll('.msg-role.assistant .role-icon-img').forEach(img=>{ img.src=src; });
  }catch(_){/* DOM not ready */}
  // Update titlebar profile avatar
  try{
    const tbAvatar=document.getElementById('titlebarProfileAvatar');
    if(tbAvatar) tbAvatar.src=src;
  }catch(_){}
}
...
function _renderUserFencedBlocks(text){
  ...
  s=s.replace(/(^|__BSN__)[ ]{0,3}(`{3,})([^__BSN__`]*)__BSN__(?:([\s\S]*?)__BSN__)?[ ]{0,3}\2`*[ \t]*(?=__BSN__|$)/g,(_,lead,_fence,info,code)=>{
...
function renderMd(raw){
  ...
  s=s.replace(/([.!?])\*\*([^*__BSN__]{1,80})\*\*__BSN____BSN__/g,'$1__BSN____BSN__**$2**__BSN____BSN__');
...
function _assistantRoleHtml(tsTitle='', tpsText=''){
  ...
  const _src=(typeof assistantAvatarSrc==='function'?assistantAvatarSrc():'static/ai.png');
  return `<div class="msg-role assistant" ${tsTitle?`title="${esc(tsTitle)}"`:''}><div class="role-icon assistant"><img src="${esc(_src)}" class="role-icon-img" alt=""></div><span class="msg-role-name">${esc(_bn)}</span>${tps}</div>`;
}
function _userRoleHtml(tsTitle=''){
  let _un='User';
  try{ _un=localStorage.getItem('hermes-username')||(window._userName||'User'); }catch(_){}
  return `<div class="msg-role user" ${tsTitle?`title="${esc(tsTitle)}"`:''}><span class="msg-role-name">${esc(_un)}</span><div class="role-icon user"><img src="static/user.png" class="role-icon-img" alt=""></div></div>`;
}
  ...
  root.querySelectorAll('.csv-inline-load:not([data-loaded])').forEach(el=>{
    ...
    fetch('api/media?path='+encodeURIComponent(path))
      ...
      .then(text=>{
        ...
        const rows=text.replace(/\r__BSN__/g,'__BSN__').replace(/\r/g,'__BSN__').split('__BSN__').filter(r=>r.trim());
        if(rows.length<2){
