...
// Welcome page quick-prompt suggestions (editable via Settings → Conversation, #welcome-prompts)
// Default icons cycle through three glyphs (folder, calendar, map) for visual variety.
const _WELCOME_PROMPT_ICONS=[
...
function _escapeHtmlForPrompt(s){
  return String(s==null?'':s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
}

function renderWelcomePrompts(prompts){
  const host=$('welcomeSuggestions');
  if(!host) return;
  const list=Array.isArray(prompts)?prompts.filter(s=>typeof s==='string'&&s.trim()):[];
  window._welcomePrompts=list.slice();
  host.innerHTML='';
  list.forEach((text,idx)=>{
    ...
    btn.innerHTML=_WELCOME_PROMPT_ICONS[idx%_WELCOME_PROMPT_ICONS.length]+' <span>'+_escapeHtmlForPrompt(text)+'</span>';
    btn.onclick=()=>{const m=$('msg');if(m){m.value=btn.dataset.msg;send();}};
...
(async()=>{
  ...
  try{
    ...
    if(typeof renderWelcomePrompts==='function') renderWelcomePrompts(Array.isArray(s.welcome_prompts)?s.welcome_prompts:[]);
    ...
    // Theme/skin/font-size are fixed: always light / default / xlarge
    const theme='light',skin='default';
    localStorage.setItem('hermes-theme',theme);
    _applyTheme(theme);
    localStorage.setItem('hermes-skin',skin);
    _applySkin(skin);
    const fontSize='xlarge';
    ...
    applyEmptyStateSuggestionPref();
    if(typeof renderWelcomePrompts==='function') renderWelcomePrompts([]);
  ...
  // Fetch active profile (and apply its avatar to chat bubbles)
  try{
    const p=await api('/api/profile/active');
    S.activeProfile=p.name||'default';
    if(typeof setProfileAvatarUrl==='function') setProfileAvatarUrl(p.avatar_url||'');
  }catch(e){S.activeProfile='default';if(typeof setProfileAvatarUrl==='function') setProfileAvatarUrl('');}
  applyBotName();
